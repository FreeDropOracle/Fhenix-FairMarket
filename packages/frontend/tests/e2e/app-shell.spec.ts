...
test("portfolio and governance surfaces render their release-ready sections", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { level: 1, name: /Protocol status, recovery language/i })).toBeVisible();
  await expect(page.getByRole("heading", { level: 2, name: /Status signals users can trust/i })).toBeVisible();
});
